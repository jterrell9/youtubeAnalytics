...
public class TopK_Views {
	...
	public static void main(String[] args) {
		//check correct usage
		if(args.length != 2) {
			System.err.println("usage: $SPARK-HOME/bin/spark-submit --class TopK_Views ./target/<jar-file> <k> <input-dir>"
		...
		SparkConf conf = new SparkConf().setAppName("Top K Viewed YouTube Videos");
		...
		PairFunction<String, String, String> mapViewsFunc = new PairFunction<String, String, String>() {
			@Override
			public Tuple2<String, String> call(String record) {
				...
				return new Tuple2<String, String>(views, video);
			}
		};
		...
		//create RDD chain
		JavaRDD<String> sortedMusicRDD = musicRDD.mapToPair(mapViewsFunc).filter(skipFilterFunc).sortByKey(false).values().persist(StorageLevel.MEMORY_AND_DISK());
		JavaRDD<String> sortedEducationRDD = educationRDD.mapToPair(mapViewsFunc).filter(skipFilterFunc).sortByKey(false).values().persist(StorageLevel.MEMORY_AND_DISK());
		JavaRDD<String> sortedEntertainmentRDD = entertainmentRDD.mapToPair(mapViewsFunc).filter(skipFilterFunc).sortByKey(false).values().persist(StorageLevel.MEMORY_AND_DISK());
		JavaRDD<String> sortedSandTRDD = scienceAndTechRDD.mapToPair(mapViewsFunc).filter(skipFilterFunc).sortByKey(false).values().persist(StorageLevel.MEMORY_AND_DISK());
		JavaRDD<String> sortedNandPRDD = newsAndPoliticsRDD.mapToPair(mapViewsFunc).filter(skipFilterFunc).sortByKey(false).values().persist(StorageLevel.MEMORY_AND_DISK());
	...
	private static void writeListToFile(List<String> list, String category) {
		...
		FileWriter output;
		try {
			output = new FileWriter(new File("./top" + k + "_" + category+ "_Views.txt"));
